...
if [[ -f ${AK2}/Image.gz-dtb ]]; then
	...
	cd ${AK2}
	zip -r9 "AceKernel-$device-$commit_number.zip" -x README.md -- *
	...
	curl -s -X POST https://api.telegram.org/bot$bot_id/sendDocument -F chat_id=$chat_id -F document="@./AceKernel-$device-$commit_number.zip" > /dev/null
